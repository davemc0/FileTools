...
if (@dbKeys and eval { require Cwd }) {
    undef $evalWarning;
    local $SIG{'__WARN__'} = sub { $evalWarning = $_[0] };
    foreach (@dbKeys) {
...
TAG:    foreach $tag (@foundTags) {
    ...
    if ($outfile) {
        ...
        if ($lineCount) {
            ...
            unlink $outfile unless $append; # don't keep empty output files
        }
        undef $comma;
    }
    ++$count;
...
sub OpenOutputFile($;@)
{
    ...
    if ($textOut) {
        ...
        unless (Open(\*OUTFILE, $outfile, $mode)) {
...
be used to match any source file name).  The following examples demonstrate
basic use of this option:
...
on single-valued tags, when writing or copying, or in a B<-if> condition.
...
2) If the argument for B<-w> does not contain a format code (%d, %f or %e),
...
from multiple source files is typically done by shell redirection, ie)
...
in I<FMT> to give the files unique names.
